...
def viz_mapa_renta_media(context, base):
    ...
    for r, isla in enumerate(islas_ordenadas):
        for c, anio in enumerate(anios_mapa):
            ...
            nombre_legible = mapping_islas.get(isla, isla)
            if c == 0: ax.text(-0.1, 0.5, nombre_legible, fontsize=10, fontweight="bold", 
    ...
    fig.suptitle("Distribución Anual de Tipologías Socioeconómicas\n"
                 r"$\it{Porcentaje\ de\ secciones\ censales\ por\ clúster\ (2021-2023)}$", 
                 fontsize=16, fontweight="bold", y=0.96) 

    fig.text(0.98, 0.95, "Fuente: INE / AEAT · Elaboración propia", 
...
def viz_mapa_ingreso_dominante(context, base):
    ...
    for r, isla in enumerate(islas):
        for c, anio in enumerate(anios):
            ...
            nombre_legible = mapping_islas.get(isla, isla)
            if c == 0: ax.text(-0.15, 0.5, nombre_legible, transform=ax.transAxes, fontweight="bold", rotation=90, va="center", ha="center")

    fig.suptitle("Fractura Territorial: Modelo de Ingreso Dominante\n"
                 r"$\it{Diferenciación\ funcional\ por\ especialización\ de\ rentas\ (2021-2023)}$", 
                 fontsize=18, fontweight="bold", y=1.02)

    fig.text(0.98, 1.02, "Fuente: INE / AEAT · Elaboración propia", 
             ha="right", va="top", fontsize=9, color="black")
...
def viz_perfil_dinamico_clusters(
    ...
    nombres_legibles = {
        "renta_media_norm": "Renta Media",
        "pct_sueldos_y_salarios_norm": "Sueldos y Salarios",
        "pct_pensiones_norm": "Pensiones"
    }
    ...
    p = (
        ...
        + scale_color_brewer(labels=nombres_legibles)
        + labs(
        ...
        + theme_bw()
        + theme(
            plot_title=element_text(size=14, weight="bold", margin={'b': 5}),
            plot_subtitle=element_text(size=10, margin={'b': 15}),
            plot_caption=element_text(size=9, ha="right"),
